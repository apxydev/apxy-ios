...
To send traffic to desktop APXY, pass a server URL plus signed ingest credentials generated by APXY Core:
...
Protected remote ingest always uses signed HTTP in the current release.

Remote configuration is grouped under `ApxyOptions.remote` so `serverURL` and ingest credentials are applied together.

Plain `http://` endpoints are allowed automatically in `DEBUG` builds for local desktop development. Non-`DEBUG` builds require HTTPS for remote delivery.
...
Groups the APXY server URL with the credentials required for signed ingest.
...
- `.webSocket`: available for local/runtime compatibility, but protected remote ingest currently falls back to signed HTTP
- `.auto`: available for local/runtime compatibility, but protected remote ingest currently falls back to signed HTTP
...
- remote runtime changes require a valid `remote` configuration
- invalid remote input is rejected and the current runtime configuration stays active
